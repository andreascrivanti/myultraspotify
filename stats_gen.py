def create_track_list(data_dict):
    ...
    for i in range(0, len(data_dict['albums'])):
        new_track = (data_dict['song_names'][i], data_dict['albums'][i], data_dict['artists'][i],  data_dict['track_ids'][i], data_dict['albums_image'][i],data_dict['artists_id'][i])
...
def calc_most_freq_played_album(tracks: list):
    ...
    ans = ()
    max_freq = 0
    album_lst = [(track[1], track[2], track[4]) for track in tracks]
...
def calc_most_freq_played_artist(tracks: list):
    ...
    ans = ()
    max_freq = 0
    artist_lst = [(track[1],track[2], track[5]) for track in tracks]
